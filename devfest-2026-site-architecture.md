...
- **Lite mode** — `?lite=1` on, `?lite=0` off, either way persisted; exposed as a "Lite version"
  toggle button (`aria-pressed`, with a visible on/off state) in the footer of every route, and
  as an opt-out inside `<IntroEscape>` so it is reachable *during* the intro rather than only
  from the bottom of the page. The toggle drives the preference through the URL so the choice is
  linkable and testable. Shares its rendering path with reduced-motion and no-JS; see Part 4 of
  the motion spec. Site-wide, not a homepage feature.
  - What lite actually drops: three.js (both the marquee and the 3D title), the hallway, the
    preloader/intro, and Lenis. GSAP and Lenis are still *statically imported* by
    `MotionProvider`, so they are still downloaded — making lite drop those bytes too needs the
    motion components to stop being imported at all (cookie + server-side branch), which is not
    done.
