...
- **Fonts** — UI type is Google Sans from the Google Fonts CSS linked in `app/layout.tsx`
  (`--font-google-sans` in `app/globals.css`). The 3D title uses
  `public/fonts/google-sans-bold.typeface.json` via Three.js `FontLoader`. Do not commit a
  full TTF and wire `next/font/local` at it — Next copies the file byte for byte.
...
- **`/` and `/memories`** — the motion work (hero hallway, memories fly-through).
